...
// Firefox: Don't restore from storage (background scripts are persistent)
// Starting fresh prevents stale tab IDs from blocking new injections
bgLog.info('Firefox: Starting with fresh injectedTabs Set');

// Clear any stale storage from previous sessions
chrome.storage.local.remove(['injectedTabs']);

// Persist injected tabs state (for debugging only, not restored)
    ...
    try {
        ...
        for (const file of files) {
            try {
                ...
                // Firefox throws "non-structured-clonable data" error for scripts
                // that export functions/objects to window. This is safe to ignore
                // as the script is still injected successfully.
                if (err.message && err.message.includes('non-structured-clonable')) {
                    bgLog.debug(`Firefox: Ignoring clonable error for ${file}`, {
                        file,
                        error: 'non-structured-clonable (expected)'
                    });
                } else {
                    // Real error - rethrow to outer catch
                    throw err;
                }
            }
        }
        ...
        case 'testPlexConnection':
            ...
            (async () => {
                try {
                    ...
                    // Try fetch() for both Chrome and Firefox
                    // Firefox might have better error messages with fetch
                    const isFirefox = typeof browser !== 'undefined';
                    bgLog.info(`🌐 Using fetch() for ${isFirefox ? 'Firefox' : 'Chrome'}`);
